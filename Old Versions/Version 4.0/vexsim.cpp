...
Task * VexSim::CheckSuspensionQueue(Node *cur)
{
	...
	{
		Total_Scheduler_Workload++; //Scheduler workload is associated with total scheduler workload required during one simulation run.
		if (cur->Area >= temp->item->NeededArea) //enough area
		{	
		    if (cur->ConfigNo != temp->item->AssignedConfig) // if the node is exact match to cpref of task, do nothing (only print a msg)
			{
				RemoveNodeFromIdleList(cur);
				cur->ConfigNo=temp->item->AssignedConfig;
				sendBitstream(cur); // else configure the current node
				AddNodeToIdleList(cur); // this is something stupid!!! extra here because the node was initially blank we first add it to the idle
...
//--------------------------------------------------------------------------------------------------------
		// Scheduling policy-3: Wait for a busy node with exact match to become available!!!
		//---------------------------------------------------------------------------------------------------------
